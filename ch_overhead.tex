...
First, it establishes a \emph{baseline}. The source frameworks supported here span the full abstraction spectrum: Dapper is a thin micro-ORM that materialises rows with almost no machinery, NHibernate is a mature full ORM with a session and a proprietary mapping engine, and Entity Framework Core is a full ORM with an expression-tree query pipeline. Quantifying what each one costs at rest tells us what a developer pays for the abstraction they chose, and therefore what they stand to gain or lose by migrating away from or toward it.
...
Measurements were collected with \texttt{BenchmarkDotNet}~\cite{benchmarkdotnet}, which handles warm-up, multi-invocation iteration, outlier removal, and the separation of one-time \acrshort{jit} costs from steady-state execution, and reports the mean, standard error, and standard deviation of each method together with managed-heap allocations and per-generation garbage-collection counts via its memory diagnoser. Reporting allocated memory alongside time is essential here: for a managed runtime, allocation pressure is a large part of an \acrshort{orm}'s true cost and a direct driver of garbage-collection pauses under load.
...
\Cref{fig:overhead-time-memory} reports the mean time overhead and allocated memory for all 23 queries on a logarithmic scale. The dominant pattern is unambiguous and consistent: across the suite, Dapper's overhead is roughly an order of magnitude below that of the two full \acrshort{orm}s. For the entity-projection query A1, Dapper materialises a row in $690$\,ns while Entity Framework Core takes $16.0$\,\textmu s and NHibernate $8.2$\,\textmu s --- a $23\times$ and $12\times$ gap respectively. Taken over the whole suite, the median per-query time overhead of Entity Framework Core is about $21\times$ Dapper's and that of NHibernate about $20\times$. The memory picture is the same shape: A1 allocates $1.1$\,KB under Dapper, $11$\,KB under Entity Framework Core, and $31$\,KB under NHibernate. NHibernate is consistently the heaviest allocator, reflecting its session and proxy machinery, even where its time overhead is competitive with Entity Framework Core's.
